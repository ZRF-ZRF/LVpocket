...
from keras.layers import Input, Conv3D, Conv3DTranspose, Concatenate, PReLU, Convolution3D, BatchNormalization, Activation, Add
...
def threshold(x):
    ...
    one = tf.ones_like(x)  # 生成与a大小一致的值全部为1的矩阵
    zero = tf.zeros_like(x)
    label = tf.where(x < 0.95, x=one, y=one)
    #print(label)
    return label
...
class LV_NET(Model):
    ...
    def identity_block_1(self, input_tensor, filters, stage, block, layer=None):
        filter1, filter2, filter3 = filters
        if K.image_data_format() == 'channels_last':  # （样本数，行数（高），列数（宽），通道数）
            bn_axis = 4
        else:
        ...
        conv_name_base = 'res' + str(stage) + block + '_branch'
        bn_name_base = 'bn' + str(stage) + block + '_branch'
        x1 = Convolution3D(filters=filter1, kernel_size=1, name=conv_name_base + '2a', kernel_regularizer=l2(1e-4))(
            input_tensor)
        print({'identity_x1': x1})
        if layer == None:
            x = BatchNormalization(axis=bn_axis, name=bn_name_base + '2a')(x1)
            print({'bn1': x})
        x = Activation('relu')(x)
        print({'relu1': x})
        x = Convolution3D(filters=filter2, kernel_size=3, padding='same', name=conv_name_base + '2b',
    ...
    def __init__(self, inputs=None, outputs=None, data_handle=None,
        ...
        if inputs is not None:
            ...
            print({'input': inputs})
            conv1 = Conv3D(16, 5, padding='same', name='conv1')(inputs)
            conv1 = PReLU(name='conv1_prelu')(conv1)  # activation
            print({'conv1': conv1})

            concat1 = Concatenate(name='concat1')([inputs, conv1])
            print({'concate1': concat1})
            down_conv1 = Conv3D(32, [2, 2 , 1], strides=2, name='down_conv_1')(concat1)
            down_conv1 = PReLU(name='down_conv1_prelu')(down_conv1)
            print({'down_conv1': down_conv1})

            conv2_1 = Conv3D(32, 5, padding='same', name='conv2_1')(down_conv1)
            ...
            print({'concate2': concat2})
            down_conv2 = Conv3D(64, [2, 2, 1], strides=2, name='down_conv2')(concat2)
            down_conv2 = PReLU(name='down_conv2_Prelu')(down_conv2)
            print({'down_conv2': down_conv2})
            ...
            conv3_3 = PReLU(name='conv3_3_relu')(conv3_3)
            print({'conv3_3': conv3_3})

            concat3 = Concatenate(name='concat3')([conv3_3, down_conv2])
            print({'concate3': concat3})
            down_conv3 = Conv3D(128,[2, 2, 2], strides=2,name='down_conv3')(concat3)
            down_conv3 = PReLU(name='down_conv3_relu')(down_conv3)
            print({'down_conv3_3': down_conv3})
            ...
            print({'concate4': concat4})
            down_conv4 = Conv3D(256, [2, 2, 1], strides=2, name='down_conv4')(concat4)
            down_conv4 = PReLU(name='down_conv4_relu')(down_conv4)
            print({'down_conv4': down_conv4})

             #LV中的‘L’
            LV_1 = Conv3D(16, 5, padding='same', name='LV-conv1')(inputs)
            LV_prelu_1 = PReLU(name='LV_prelu1')(LV_1)

            LV_concat1 = Concatenate(name='LV-concat1')([inputs, LV_prelu_1])

            LV_down_1 = Conv3D(32, [2, 2 , 1], strides=2, name='LV-down1')(LV_concat1)
            LV_prelu_2 = PReLU(name='LV-prelu2')(LV_down_1)

            LV_2 = Conv3D(32, 5, padding='same', name='LV-conv2')(LV_prelu_2)
            LV_prelu_3 = PReLU(name='LV-prelu3')(LV_2)

            LV_concat2 = Concatenate(name='LV-concat2')([LV_prelu_2, LV_prelu_3])

            LV_down_2 = Conv3D(64, [2, 2, 1], strides=2, name='LV-down2')(LV_concat2)
            LV_prelu_4 = PReLU(name='LV-prelu4')(LV_down_2)

            LV_3 = Conv3D(64, 5, padding='same', name='LV-conv3')(LV_down_2)
            LV_prelu_5 = PReLU(name='LV-prelu5')(LV_3)

            LV_concat3 = Concatenate(name='LV-concat3')([LV_prelu_4, LV_prelu_5])

            LV_down_3 = Conv3D(128,[2, 2, 2], strides=2,name='LV-down3')(LV_concat3)
            LV_prelu_6 = PReLU(name='LV-prelu6')(LV_down_3)

            LV_4 = Conv3D(128,5,padding='same',name='LV-conv4')(LV_prelu_6)
            LV_prelu_7 = PReLU(name='LV-prelu7')(LV_4)

            LV_concat4 = Concatenate(name='LV-concat4')([LV_prelu_7, LV_prelu_6])

            LV_down_4 = Conv3D(256, [2, 2, 1], strides=2, name='LV-down4')(LV_concat4)
            LV_prelu_8 = PReLU(name='LV-prelu8')(LV_down_4)

            LV_5 = Conv3D(256, 5, padding='same', name='LV-conv5')(LV_prelu_8)
            LV_prelu_9 = PReLU(name='LV-prelu9')(LV_5)



            conv5_1 = Conv3D(256, 5, padding='same', name='conv5_1')(down_conv4)
            conv5_1 = PReLU(name='conv5_1_relu')(conv5_1)
            conv5_2 = Conv3D(256, 5, padding='same', name='conv5_2')(conv5_1)
            conv5_2 = PReLU(name='conv5_2_relu')(conv5_2)
            conv5_3 = Conv3D(256, 5, padding='same', name='conv5_3')(conv5_2)
            conv5_3 = PReLU(name='conv5_3_relu')(conv5_3)
            print({'conv5_3': conv5_3})

            concat5 = Concatenate(name='concat5')([down_conv4, conv5_3, LV_prelu_9])
            print({'concat5': concat5})
            up_conv5 = Conv3DTranspose(256, [2, 2, 1], strides=2, name='up_conv5',kernel_regularizer=l2(l2_lambda))(concat5)
            up_conv5 = PReLU(name='up_conv5_relu')(up_conv5)
            print({'up_conv5': up_conv5})

            concat4_1 = self.identity_block(concat4, [256, 256, 256], stage=1, block='c')
            print({'concat4_1': concat4_1})
            conv6 = Concatenate(name='conv6')([up_conv5, concat4_1])
            print({'conv6': conv6})
            conv6_1 = Conv3D(256, 5, padding='same', name='conv6_1')(conv6)
            conv6_1 = PReLU(name='conv6_1_relu')(conv6_1)
            conv6_2 = Conv3D(256, 5, padding='same', name='conv6_2')(conv6_1)
            conv6_2 = PReLU(name='conv6_2_relu')(conv6_2)
            conv6_3 = Conv3D(256, 5, padding='same', name='conv6_3')(conv6_2)
            ...
            up_conv6 = Conv3DTranspose(128, [3 , 3, 3], strides=2,name='up_conv6')(concat6)
            up_conv6 = PReLU(name='up_conv6_relu')(up_conv6)
            print({'up_conv6': up_conv6})

            concat3_1 = self.identity_block(concat3, [128, 128, 128], stage=2, block='d')
            print({'concate3_1': concat3_1})
            conv7 = Concatenate(name='conv7')([up_conv6, concat3_1])
            print({'conv7': conv7})
            conv7_1 = Conv3D(128, 5,padding='same',name='conv7_1')(conv7)
            conv7_1 = PReLU(name=f'conv7_1_relu')(conv7_1)
            conv7_2 = Conv3D(128, 5, padding='same', name='conv7_2')(conv7_1)
            conv7_2 = PReLU(name='conv7_2_relu')(conv7_2)
            conv7_3 = Conv3D(128, 5, padding='same', name='conv7_3')(conv7_2)
            conv7_3 = PReLU(name='conv7_3_relu')(conv7_3)
            print({'conv7_3': conv7_3})

            concat7 = Concatenate(name='concat7')([up_conv6, conv7_3])
            print({'concat7': concat7})
            up_conv7 = Conv3DTranspose(64, [2, 2, 1], strides=2,name='up_conv7')(concat7)
            up_conv7 = PReLU(name='up_conv7_relu')(up_conv7)
            print({'up_conv7': up_conv7})

            concat2_1 = self.identity_block(concat2, [64, 64, 64], stage=3, block='e')
            print({'concat2_1': concat2_1})
            conv8 = Concatenate(name='conv8')([up_conv7, concat2_1])
            print({'conv8': conv8})
            conv8_1 = Conv3D(64, 5, padding='same', name='conv8_1')(conv8)
            conv8_1 = PReLU(name='conv8_1_relu')(conv8_1)
            conv8_2 = Conv3D(64, 5, padding='same', name='conv8_2')(conv8_1)
            conv8_2 = PReLU(name='conv8_2_relu')(conv8_2)
            print({'conv8_2': conv8_2})

            concat8 = Concatenate(name='concat8')([conv8_2, up_conv7])
            print({'concat8': concat8})
            up_conv8 = Conv3DTranspose(32, [2, 2, 1], strides=2, name='up_conv8')(concat8)
            up_conv8 = PReLU(name='up_conv8_relu')(up_conv8)
            print({'up_conv8': up_conv8})

            # concat1_1 = Conv3D(32, [2, 2, 1], strides=1, padding='same', name='down_conv1_2')(concat1)
            #concat1_1 = self.identity_block_1(concat1, [32, 32, 32], stage=4, block='f')

            conv9 = Concatenate(name='conv9')([up_conv8, concat1])
            print({'conv9': conv9})
            conv9_1 = Conv3D(32, 5, padding='same', name='conv9_1')(conv9)
            print({'conv9_1': conv9_1})

            concat9 = Concatenate(name='concat9')([conv9_1, up_conv8])
            print({'concat9': concat9})
            embedding = tf.get_default_graph().get_tensor_by_name('concat9/concat:0')
