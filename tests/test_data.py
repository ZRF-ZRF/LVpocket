...
@pytest.mark.parametrize("num_steps", (1, 3, 10), ids=lambda x: "%s steps" % x)
def test_print_progress(capsys, num_steps):
    ...
    printed_str = "".join(
        "%s elements processed\n" % i for i in range(0, 100, num_steps)
    )
    assert out == printed_str


@pytest.mark.parametrize(
    "ids", (None, "all", "half"), ids=("no IDs", "all IDs", "half of IDs")
)
@pytest.mark.parametrize(
...
def test_prepare_dataset(ids, dataset_path, db_format):
    keys = ("coords", "features", "centroid", "pocket_coords", "pocket_features")
    ...
    with tempfile.NamedTemporaryFile(suffix=".hdf") as tmp:
        ...
        with h5py.File(tmp.name, "r") as f:
            for idx in stored_ids:
                ...
                assert len(f[idx]["pocket_features"]) == len(f[idx]["pocket_coords"])
...
@pytest.mark.parametrize(
    ...
    ids=("negative", "too big", "wrong shape", "wrong type"),
)
...
def test_sample_generator(max_dist, scale, footprint):
    ...
    data = DataWrapper(
        test_prepared_dataset,
        pdbids=pdbids,
        test_set=test_pdbids,
        load_data=True,
        footprint=footprint,
        max_dist=max_dist,
        scale=scale,
    ...
    box = ceil(2.0 * max_dist / (1.0 / scale) + 1)
    ...
    generator = data.sample_generator("test", transform=False, random_order=False)
