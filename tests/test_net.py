...
@pytest.mark.parametrize("smoothing", (0, 0.1, 0.001), ids=lambda x: "smoothing %s" % x)
def test_dice(smoothing):
    x = Input(input_shape[1:])
    m = Model(inputs=x, outputs=x)

    arr_random = np.random.choice([0, 1], size=input_shape, p=[0.75, 0.25])
...
def test_lvnet_from_layers(box_size, i, o):
    ...
    conv1 = Convolution3D(filters=3, kernel_size=1, activation="elu", padding="same")(
        inputs
    )
    outputs = Convolution3D(
        filters=o, kernel_size=1, activation="sigmoid", padding="same"
    )(conv1)
...
def test_lvnet_with_featurizer(box_size, o):
    ...
    model = LV_NET(
        box_size=box_size, input_channels=i, output_channels=o, scale=0.5, featurizer=f
    )
    assert hasattr(model, "data_handle")
    assert model.data_handle is None
...
def test_multiple_inputs_outputs(box_size, i_channels, o_channels):
    ...
    conv1 = [
        Convolution3D(filters=3, kernel_size=1, activation="elu", padding="same")(inp)
        for inp in inputs
    ]
    conv1 = concatenate(conv1, axis=-1)
    outputs = [
        Convolution3D(filters=o, kernel_size=1, activation="sigmoid", padding="same")(
            conv1
        )
        for o in o_channels
    ]
...
def test_training(data, loss):
    ...
    outputs = Convolution3D(
        filters=output_channels, kernel_size=1, activation="sigmoid"
    )(inputs)

    model = LV_NET(inputs=inputs, outputs=outputs)
    model.compile(
        optimizer=Adam(lr=1e-6), loss=loss, metrics=[dice, dice_loss, ovl, ovl_loss]
    )
    model.fit_generator(train_gen, steps_per_epoch=2, epochs=num_epochs, verbose=0)

    for scores in (
        model.evaluate_generator(eval_gen, steps=2),
...
@pytest.mark.parametrize(
    ...
    ids=(
        "wrong scale",
        "wrong dist",
        "wrong featurizer type",
        "wrong featurizer shape",
    ),
)
@pytest.mark.parametrize("compiled", (True, False), ids=("compiled", "not compiled"))
...
@pytest.mark.parametrize(
    ...
    (
        ...
        {
            "outputs": Convolution3D(
                filters=3, kernel_size=1, activation="elu", padding="same"
            )(Input([36] * 3 + [1]))
        },
    ),
    ids=(
...
def test_incompatible_with_layers(kwargs):
    ...
    outputs = Convolution3D(
        filters=5, kernel_size=1, activation="sigmoid", padding="same"
    )(conv1)
    with pytest.raises(ValueError, match=list(kwargs)[0]):
...
def test_save_pockets_cmap(data):
    ...
    with pytest.raises(ValueError, match="saving more than one prediction"):
        model.save_density_as_cmap(np.concatenate((density, density)), origin, step)
    ...
    with tempfile.NamedTemporaryFile(suffix=".cmap") as cmap_file:
        ...
        with h5py.File(fname, "r") as f:
            ...
            for i in range(data.y_channels):
                ...
                assert np.allclose(density[0, ..., i].transpose([2, 1, 0]), dataset[:])
...
def test_save_pockets_cube(data):
    ...
    with pytest.raises(ValueError, match="saving more than one prediction"):
        model.save_density_as_cube(np.concatenate((density, density)), origin, step)
...
def test_predict_mol(box_size, o):
    ...
    with pytest.raises(ValueError, match="featurizer must be set"):
        model = LV_NET(
            box_size=box_size, scale=0.5, input_channels=num_features, output_channels=o
        )
    ...
    model = LV_NET(
        featurizer=featurizer, box_size=box_size, scale=0.5, output_channels=o
    )
    model.compile(optimizer=Adam(lr=1e-6), loss="binary_crossentropy")
...
def test_predict_pocket_atoms(box_size, o):
    ...
    pocket_mols_atoms = model.predict_pocket_atoms(
        mol, dist_cutoff=3, expand_residue=False, **segmentation_kwargs
    )
    pocket_mols_residues = model.predict_pocket_atoms(
        mol, dist_cutoff=3, expand_residue=True, **segmentation_kwargs
    )
