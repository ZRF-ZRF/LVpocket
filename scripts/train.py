...
from model.LV_former import LV_NET, dice_loss, dice, ovl
...
def parse_args():
    ...
    parser.add_argument(
        "--load", "-l", action="store_true", help="whether to load all data into memory"
    )
    parser.add_argument(
        "--output",
        "-o",
    ...
    parser.add_argument(
        "--verbose", "-v", default=2, type=int, help="verbosity level for keras"
    )
...
def main():
    ...
    if not os.access(args.output, os.W_OK):
        raise IOError(
            "Cannot create files inside %s (check your permissions)." % args.output
        )
    ...
    data = DataWrapper(
        args.input, test_set=test_ids, pdbids=all_ids, load_data=args.load
    )

    if args.model:
        model = LV_NET.load_model(args.model, data_handle=data)
    else:
    ...
    callbacks = [
        ModelCheckpoint(
            os.path.join(args.output, "checkpoint.hdf"), save_best_only=False
        )
    ]

    if test_ids:
        val_batch_generator = data.batch_generator(
            batch_size=args.batch_size, subset="test"
        )
        num_val_steps = max(args.steps_per_epoch // 5, 1)
        callbacks.append(
            ModelCheckpoint(
                os.path.join(args.output, "best_weights.hdf"), save_best_only=True
            )
        )
    else:
