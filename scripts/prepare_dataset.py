...
def output_path(path):
    ...
    if not os.access(dirname, os.W_OK):
        raise IOError("File %s cannot be created (check your permissions)." % path)
    return path
...
def parse_args():
    ...
    parser.add_argument(
        "--dataset", "-d", required=True, type=input_path, help="path to the database"
    )
    parser.add_argument(
        "--include",
        "-i",
