...
from model.LV_former import LV_NET
...
def parse_args():
    ...
    parser.add_argument(
        "--verbose", "-v", action="store_true", help="whether to print messages"
    )
    parser.add_argument(
        "--max_dist",
        type=float,
    ...
    parser.add_argument(
        "--scale", type=float, default=0.5, help="scale parameter used for training set"
    )
...
def main():
    ...
    for path in progress_bar(args.input):
        ...
        model.save_density_as_cmap(
            density, origin, step, fname=os.path.join(dirname, "pockets.cmap")
        )
