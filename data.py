...
def print_progress(iterable, num_steps=10):
    ...
    # enumerate:给定一个可迭代对象，可选值start, 返回一个iterator的索引，值的元组。
    ...
    # yield类似return的关键字,把yield想象成return,return了之后，程序停止，第二次调用从刚才那个next程序停止的地方开始执行的,执行再次遇到yield关键字结束

def _pdbbind_paths(data_path, idx):
    return ('%s/%s/ligand.mol2' % (data_path, idx),
...
def _site_paths(data_path, idx):
    return ('%s/%s/site.mol2' % (data_path, idx),
            '%s/%s/protein.mol2' % (data_path, idx))
...
def _get_binary_features(mol):
    ...
    features = np.ones((len(coords), 1))  #生成值为1的一维数组，长度为蛋白质中的原子数量
...
def prepare_dataset(data_path, protein_featurizer, pocket_featurizer=None,
    ...
    if db_format == 'scpdb':
        ...
        #将structure_id中的_0-9结束的替换为空格
    elif db_format == 'pdbbind':
        get_paths = _pdbbind_paths
        get_id = lambda structure_id: structure_id
    elif db_format == 'site':
        get_paths = _site_paths
        get_id = lambda  structure_id: structure_id
    else:
    ...
    with h5py.File(hdf_path, mode=hdf_mode) as f:
        ...
        for structure_id in progress_bar(ids):
            # progress_bar：print index,value
...
class DataWrapper():
    ...
    def __init__(self, hdf_path, pdbids=None, test_set=None, load_data=False,
        ...
        if test_set is not None:
            if isinstance(test_set, (set, tuple, list, np.ndarray)):
                # 判断一个函数是否是一个已知类型，如果对象的类型与参数二的类型相同返回true，否则false
        ...
        if footprint is not None:
            ...
            if isinstance(footprint, int):
                if footprint == 0:
                    ...
                    # ellipsoid生成半长轴与网格上具有指定“间距”的网格尺寸对齐的椭圆体,xyz长度
    ...
    def _open_data_handle(self):
        if hasattr(self, 'closed') and not self.closed:
            # it's open，hasattr返回对象是否具有给定名称的属性
            return
    ...
    def prepare_complex(self, pdbid, rotation=0, translation=(0, 0, 0),
        ...
        vmin, vmax: floats, optional (default 0 and 1)
            Clip values generated for pocket to this range 口袋范围
        ...
        # 计算多维最大滤波器,maximum_filter(input,size)
        # 返回Filtered array，Has the same shape as `input`
        pocket_dens += margin
    ...
    def sample_generator(self, subset='training', transform=True,
        ...
        while True:
            if random_order:
                shuffle(pdbids)
        #random.shuffle()函数的作用是将原列表中的元素打乱顺序，并不会产生新的列表
